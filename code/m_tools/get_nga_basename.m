...
%
% OUT   fileName = original wform file-name but without directory-name and extension
...
nc = 3;   % No. of elements of orientation code. Default value = 3, in case no orntCode can be identified
          % = number of characters that will be stripped off raw name for
          % constructing base name
np = 0;   % No. of times a name has been processed (to detect multiple processings = error)
          % Should be = 1 if everything went well
...
% a. vertical    -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -
z_orntCodes = {'-UP';'-V1';'DN';'DWN';'--Z';'--V';'-V';'UP1'};
...
% b. horizontal     -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -
hn_orntCodes = {'-NS';'-SN';'NS';'SN';'-N';'-S'};         % n/s orientations
he_orntCodes = {'-EW';'-WE';'EW';'WE';'-W';'-E'};         % e/w orientations
...
if (~isempty(ho_idx))
    ...
    orntCode = strcat(strng{1},'777');           % keep letter of channal description in order to tell
    nc       = numel(code);                      % apart the two horizontal components
    np       = np+1;
end
if (~isempty(ha_idx))

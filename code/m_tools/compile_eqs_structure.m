    ...
    if (trList.eq.idx(itr)==0)
        ...
        for im = 1:numel(mVals)
            ...
            if ~isempty(skipList); itmps                = find(mVals(im) == skipList.eq.m(sids_thisEvent));
                 ...
                 fprintf(1,sprintf('id = %i --- %i  records (+%i skipped) --- %s --- trList-index = %i/%i\n',id,numel(idx),numel(idxs),eqName,itr,ntr))
            else fprintf(1,sprintf('id = %i --- %i  records --- %s --- trList-index = %i/%i\n',id,numel(idx),eqName,itr,ntr))
            end
        end
    end
end

...
while itr<ntr
    ...
    idxCandidates = find(TraceList.eq.m==TraceList.eq.m(itr));
    candidateList = TraceList.selectSubList(idxCandidates);
    [iZ,iE,iN]    = find_corec_idx(fullName,candidateList,1);
    iZ            = idxCandidates(iZ);
    iE            = idxCandidates(iE);
    iN            = idxCandidates(iN);
    iAll          = sort([iZ;iE;iN]);

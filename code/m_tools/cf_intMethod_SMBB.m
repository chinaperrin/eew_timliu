function cf_intMethod_SMBB(catList)
...
for imatch = 1:nmatch
    ...
    % BROAD BAND
    [sbb1,meta_bb1] = read_any_trace_bak(catList.fullName{ibb},catList,1);      % --> the OLD way
    [sbb2,meta_bb2] = read_any_trace    (catList.fullName{ibb},catList,1);      % --> the NEW way
    
    t1     = meta_bb1.t;
    t2     = meta_bb2.t;
    ppxIdx = meta_bb1.ppxIdx;
    tppx   = t1(ppxIdx);
    
    figure(11); clf; hold on; grid on; box on;
    plot(t1,sbb1.dsp,'b','lineWidth',2)
    plot(t2,sbb2.dsp,'r','lineWidth',1)
    set(gca,'xlim',[tppx-3 tppx+10])
    
    
    % STRONG MOTION
    [ssm1,meta_sm1] = read_any_trace_bak(catList.fullName{ism},catList,1);      % --> the OLD way
    [ssm2,meta_sm2] = read_any_trace    (catList.fullName{ism},catList,1);      % --> the NEW way
    
    t1     = meta_sm1.t;
    t2     = meta_sm2.t;
    ppxIdx = meta_sm1.ppxIdx;
    tppx   = t1(ppxIdx);
    
    figure(12); clf; hold on; grid on; box on;
    plot(t1,ssm1.dsp,'b','lineWidth',2)
    plot(t2,ssm2.dsp,'r','lineWidth',1)
    set(gca,'xlim',[tppx-3 tppx+10])

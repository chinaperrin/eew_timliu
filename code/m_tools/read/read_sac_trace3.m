function [out] = read_sac_trace3(traceFullName)
...
if ~flgSkip
    % Read data
    S    = rsac(traceFullName);
    sraw = S.amplitudes;
    t    = S.timevector';
    %     dt   = roundn(S.header(1),-4);    % --> up to i36
    %     sr   = 1/dt;
    dt   = S.header(1);
    sr   = round(1/dt);
    
    %if (sr>999); fprintf(1,'8UNG: rounding is not appropriate for such sampling rates.\n'); end
    ...
    out.stLat = S.header(32);
    out.stLon = S.header(33);
    out.stEl  = S.header(34);
    out.stDep = S.header(35);
    ...
    twindow     = S.header(6);                                                      % dt between ref time and 
    mdate_start = mdate_ref+ twindow/86400;                                         % start of recording [sec]

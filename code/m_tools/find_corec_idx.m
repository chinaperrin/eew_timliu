...
elseif strcmp(extension,'.AT2')             % - - - - - - - - - - NGA West1
    ...
    usc_idx       = regexp(traceName,'_');
    recordName    = traceName(1:usc_idx(end));
    ...
    % Find out which trace is which 
    names   = traceList.fullName(idxCorec);
    matches = regexp(names,'_V\.', 'end');
    i       = find(cellfun(@(x) ~isempty(x),matches));
    ...
    if (~isempty(i))
        ...
        if (numel(idxH)==2);    idx_N   = idxH(1); 
                                idx_E   = idxH(2);
        elseif (numel(idxH)==1) idx_N   = idxH(1);
                                flg_noE = true;
        elseif (numel(idxH)==0) flg_noN = true;
                                flg_noE = true;
        end
    end
